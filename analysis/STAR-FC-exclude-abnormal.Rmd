---
title: "Limma-voom paired analysis EXCLUDING samples with ultrasound abnormality"
output:
  html_document:
    df_print: paged
---
...
```{r, message=FALSE, echo=FALSE}
rawPE <- read_delim(here("data/counts-pe/counts.txt"), delim = "\t", skip = 1)
rawSE <- read_delim(here("data/counts-se/counts.txt"), delim = "\t", skip = 1)
...
samps1 <- read_csv(here("data/CMV-AF-database-final-included-samples.csv"))
...
read_csv(file = here("data/metadata.csv")) %>%
  ...
  dplyr::select(id, 
                ...
                indication) %>%
  left_join(batch) %>%
  dplyr::filter(id %in% colnames(dge)) -> targets
...
cont <- makeContrasts(contrasts = "neg - pos",
                      levels = design)
fit <- lmFit(v, design)
...
fitSum
```
There were `r fitSum["Down",]` down-regulated and `r fitSum["Up",]` up-regulated genes between CMV negative and CMV positive samples at FDR < 0.05.  

These are the top 10 differentially expressed genes.

```{r, echo=FALSE, message=FALSE}
top <- topTable(fit2, n = 500)
readr::write_csv(top, path = here("output/star-fc-limma-voom-no_us_ab.csv"))
head(top, n = 10)
```
...
cut <- 0.05
aheatmap(v$E[v$genes$Geneid %in% top$Geneid[top$adj.P.Val < cut],], 
...
head(hCam, n = 20)
```
Barcode plots show the enrichment of gene sets among up or down-regulated genes. The following barcode plots show the enrichment of the top 4 hallmark gene sets among the genes differentially expressed between CMV negative and CMV positive samples.
...
The following barcode plots show the enrichment of the top 4 curated gene sets among the genes differentially expressed between CMV negative and CMV positive samples.
...
Test the specialised brain development gene set.
...
```{r, echo=FALSE, message=FALSE}
bsIdx <- ids2indices(list(BRAIN_DEV = alias2Symbol(pull(brainSet, var = 1))), 
                     v$genes$symbol)
bsRst <- roast(v, bsIdx, design, contrast = cont)

bsRst
```

```{r, echo=FALSE, message=FALSE}
barcodeplot(fit2$t[,1], bsIdx[[1]], main = names(bsIdx), cex.main = 0.75)
```

# Summary

Although the effective library sizes were low, the data is generally of good quality. We found at total of `r sum(abs(decideTests(fit2, p.value = 0.05)[,1]))` differentially expressed genes at FDR < 0.05. The significant genes were enriched for GO terms associated with interferon response and similar. Further gene set testing results indicate an upregulation of interferon response genes in the CMV positive samples, relative to the CMV negative samples, which is consistent with the top genes from the DE analysis.
