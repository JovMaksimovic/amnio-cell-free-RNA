...
files <- list.files(here("data/190717_A00692_0021_AHLLHFDSXX/new_salmon/quants"), 
...
Exclude CMV11 and its pairs.

```{r}
targets <- targets[targets$matchedpair != "NC1",]
files <- files[names(files) %in% targets$ID.post.extraction]
```
...
# par(mar=c(5,3,4,2)+0.1)
# plotRLE(cpm(y), las=2, col=pal[factor(samples$TestResult)], 
...
write.csv(topLV, file = here("output/exclude-CMV11/salmon-limma-voom.csv"))
head(topLV, n=10)
```
...
Testing for enrichment of GO  categories for top 100 ranked genes.

```{r, echo=FALSE}
go <- goana(topLV$entrezid[1:100], universe = y$genes$entrezid, trend="length")
topGO(go)
write.csv(topGO(go), file = here("output/exclude-CMV11/salmon-limma-voom-GO.csv"))
```
...
write.csv(c5Cam[c5Cam$FDR < 0.05,], file = here("output/exclude-CMV11/salmon-limma-voom-c5Cam.csv"))
head(c5Cam, n=10)
```
...
write.csv(hCam[hCam$FDR < 0.05,], file = here("output/exclude-CMV11/salmon-limma-voom-hCam.csv"))
head(hCam, n=10)
```
...
write.csv(c2Cam[c2Cam$FDR < 0.05,], file = here("output/exclude-CMV11/salmon-limma-voom-c2Cam.csv"))
head(c2Cam, n=10)
```
...
write.csv(keggCam[keggCam$FDR < 0.05,], file = here("output/exclude-CMV11/salmon-limma-voom-keggCam.csv"))
head(keggCam, n=10)
```
```{r, fig.width=9, fig.height=6, echo=FALSE}
par(mfrow=c(2,2))
...
The GONE gene set is not significantly enriched for differentially expressed genes in our data.

```{r}
gone <- read_delim(file = here("data/GONE4.10.13.txt"), delim = "\t")
gonePmids <- select(org.Hs.eg.db, keys = as.character(unique(gone$ID[!is.na(gone$ID)])), 
                    columns = c("PMID", "ENTREZID"), keytype = "PMID")
goneIdx <- ids2indices(gonePmids$ENTREZID, v$genes$entrezid)
roast(v, goneIdx, design, contrast = cont, trend.var = TRUE)
```

## IPA gene set

The IPA gene set is significantly enriched for differentially expressed genes in our data.

```{r}
ipa <- read_csv(file = here("data/IPA-molecule-summary.csv"))
ipaIdx <- ids2indices(ipa$Ensembl, rownames(v))
roast(v, ipaIdx, design, contrast = cont, trend.var = TRUE)
```
