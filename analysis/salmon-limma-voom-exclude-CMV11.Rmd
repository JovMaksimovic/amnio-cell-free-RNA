...
write.csv(topLV, file = here("output/salmon-limma-voom-exclude-CMV11.csv"))
head(topLV, n=10)
```
...
topGO(go)
write.csv(topGO(go), file = here("output/salmon-limma-voom-GO-exclude-CMV11.csv"))
```
...
write.csv(c5Cam[c5Cam$FDR < 0.05,], file = here("output/salmon-limma-voom-c5Cam-exclude-CMV11.csv"))
head(c5Cam, n=10)
```
...
write.csv(hCam[hCam$FDR < 0.05,], file = here("output/salmon-limma-voom-hCam-exclude-CMV11.csv"))
head(hCam, n=10)
```
...
write.csv(c2Cam[c2Cam$FDR < 0.05,], file = here("output/salmon-limma-voom-c2Cam-exclude-CMV11.csv"))
head(c2Cam, n=10)
```
...
write.csv(keggCam[keggCam$FDR < 0.05,], file = here("output/salmon-limma-voom-keggCam-exclude-CMV11.csv"))
head(keggCam, n=10)
```
```{r, fig.width=9, fig.height=6, echo=FALSE}
par(mfrow=c(2,2))
