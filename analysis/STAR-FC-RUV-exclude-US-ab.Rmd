---
title: "Limma-voom paired analysis of EXCLUDING US-ab samples"
output:
  html_document:
    df_print: paged
---
...
readr::write_csv(top, path = here("output/star-fc-ruv-no_us_ab.csv"))
head(top, n=10)
```
...
c5Cam <- camera(cpm(norm$normalizedCounts, log = TRUE), 
                gsAnnots$c5@idx, design, contrast = cont[,1], trend.var = TRUE)
write.csv(c5Cam[c5Cam$FDR < 0.05,], 
          file = here("output/star-fc-ruv-no_us_ab-gsea-c5.csv"))
head(c5Cam, n = 20)
```
...
hCam <- camera(cpm(norm$normalizedCounts, log = TRUE), 
               gsAnnots$h@idx, design, contrast = cont[,1], trend.var = TRUE)
head(hCam, n = 20)
```
...
c2Cam <- camera(cpm(norm$normalizedCounts, log = TRUE), 
                gsAnnots$c2@idx, design, contrast = cont[,1], trend.var = TRUE)
write.csv(c2Cam[c2Cam$FDR < 0.05,], 
          file = here("output/star-fc-ruv-no_us_ab-gsea-c2.csv"))
head(c2Cam, n = 20)
```
...
keggCam <- camera(cpm(norm$normalizedCounts, log = TRUE), 
                  gsAnnots$kegg@idx, design, contrast = cont[,1],
                  trend.var = TRUE)
head(keggCam, n = 20)
```
