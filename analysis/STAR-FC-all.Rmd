...
```{r}
foo <- read.table(here("data/sorted/rRNA_counts.txt"), header = FALSE,
                  stringsAsFactors = FALSE)
rownames(foo) <- sapply(1:nrow(foo), function(i) 
  paste(foo[i,1:4], collapse = "."))
foo <- foo[,13:72]
summary(colSums(foo))

blah <- read.table(here("data/deduped/rRNA_counts_deduped.txt"), header = FALSE,
                  stringsAsFactors = FALSE)
rownames(blah) <- sapply(1:nrow(blah), function(i) 
  paste(blah[i,1:4], collapse = "."))
blah <- blah[,13:72]
summary(colSums(blah))
```
